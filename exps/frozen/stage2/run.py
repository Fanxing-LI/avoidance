...
def parse_args():
    ...
    parser.add_argument('--comment', '-c', type=str, default="regulate")
    ...
    parser.add_argument("--regulate", "-r", type=bool, default=True, )
...
def main(debug_env=False):
    ...
    children_policy_cfg = load_yaml_config(current_folder + f"/../stage1/{args.chi_policy}.yaml")\
        ["algorithm"]["policy_kwargs"]

    if not args.train:
        env_config["eval_env"]["visual"] = True

    # config["algorithm"]["policy_kwargs"]["children_cfgs"] = children_policy_cfg
