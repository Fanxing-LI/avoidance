...
def parse_args():
    ...
    parser.add_argument("--scale", type=float, default=1.0, help="Scale factor for obstacles (default: 1.0)")
    return parser
...
def main(debug_env=False):
    ...
    # Apply scale to scene_kwargs
    if "scene_kwargs" not in env_config["env"]:
        env_config["env"]["scene_kwargs"] = {}
    if "scene_kwargs" not in env_config["eval_env"]:
        env_config["eval_env"]["scene_kwargs"] = {}
    env_config["env"]["scene_kwargs"]["object_scale"] = args.scale
    env_config["eval_env"]["scene_kwargs"]["object_scale"] = args.scale
    ...
    if args.train:
        ...
        test_handle.test(**config["test"], deterministic=True, sample=False)
