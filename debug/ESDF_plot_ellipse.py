...
def main():
	...
	# Single circle obstacle (ESDF)
	center = (1, -0.0)
	radius = 0.3
	esdf = compute_esdf_circle(grid_x, grid_y, center, radius)
	esdf_outside = np.ma.array(esdf, mask=esdf <= 0.0)

	# Ellipse-based gradient field (direction only)
	a = 1.5
	b = 0.3
	dx = grid_x - center[0]
	dy = grid_y - center[1]
	ellipse_level = (dx / a) ** 2 + (dy / b) ** 2 - 1.0
	ellipse_field = np.sqrt((dx / a) ** 2 + (dy / b) ** 2) - 1.0
	grad_x = 2.0 * dx / (a * a)
	grad_y = 2.0 * dy / (b * b)
	...
	fig.savefig(
		"/home/lfx-desktop/files/avoidance/debug/esdf_elllipse.png",
		dpi=600,
		bbox_inches="tight",
		pad_inches=0,
		facecolor="white",
